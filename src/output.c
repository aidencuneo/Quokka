...
string StringFromInt(int value){string out;char newval[11];sprintf(newval, "%d", value);out.value = newval;out.length = strlen(out.value);}
string StringFromLong(long value){string out;char newval[20];sprintf(newval, "%ld", value);out.value = newval;out.length = strlen(out.value);}
string StringFromChar(char value){string out;char newval[2] = "\0";newval[0] = value;out.value = newval;out.length = strlen(out.value);return out;}
string StringFromCharPointer(char * value){string out;out.value = value;out.length = strlen(value);return out;}

#define String(value) _Generic((value),    int : StringFromInt,    long : StringFromLong,    char : StringFromChar,    char * : StringFromCharPointer,    default : StringFromCharPointer)(value)


string stringlstrip(string s){if(!s.value)return s;while(s.value[0]==' ')s.value++;return s;}
char*stringrstrip(char*s){if(!s)return s;size_t size;char*end;size=strlen(s);if(!size)return s;end=s+size-1;while(end>=s&&isspace(*end))end--;*(end+1)='\0';return s;}
char*stringstrip(char*s){if(!s)return s;size_t size;char*end;size=strlen(s);if(!size)return s;end=s+size-1;while(end>=s&&isspace(*end))end--;*(end+1)='\0';while(*s&&isspace(*s))s++;return s;}
...
char*cpstrip(char*s){if(!s)return s;size_t size;char*end;size=strlen(s);if(!size)return s;end=s+size-1;while(end>=s&&isspace(*end))end--;*(end+1)='\0';while(*s&&isspace(*s))s++;return s;}

void tokenise(char*arr[],char*buffer,char*tokAt)
{    int i = 0;    arr[0] = strtok(buffer, tokAt);    while (arr[i] != NULL)    {        i++;        arr[i] = cpstrip(strtok(NULL, tokAt));    }}
void ntokenise(char*arr[],char*buffer,char*tokAt){    int i = 0;    arr[0] = nstrtok(buffer, tokAt);    while (arr[i] != NULL)    {        i++;        arr[i] = cpstrip(nstrtok(NULL, tokAt));    }}
...
#define print(value) _Generic((value),    int : iprint,    long : lprint,    char : cprint,    char * : cpprint,    string : sprint,    default : sprint)(value)
...
#define println(value) _Generic((value),    int : iprintln,    long : lprintln,    char : cprintln,    char * : cpprintln,    string : sprintln,    default : sprintln)(value)
